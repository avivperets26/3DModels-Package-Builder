...
|---|---|---|---|---|---|---|
| PB-0004 | 🟡 **PROCESS** | `chore/PB-0004-gitignore` | Codex for local correction; user for Git and GitHub gates | 2026-07-23 | The prior policy checkpoint was merged through PR #5 as `cab7c3cbf803bf8f1c6187c2ee18dc5f08717988`, but a standalone Windows PowerShell parameter-binding failure was subsequently verified. The corrective implementation now derives an omitted repository root from the executing script path and adds a child-process regression check. Current local runs pass 107/107 ignore-policy cases (67 ignored, 40 trackable), check 15 tracked paths with no conflict, and pass all 14 repository-baseline checks. | The corrective changes have not been committed, pushed, validated by pull-request CI, merged into `main`, validated by final `main` CI, or explicitly confirmed complete by the user. |
