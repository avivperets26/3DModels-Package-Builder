...
|---|---|---|---|---|---|---|
| PB-0201 | 🟡 **PROCESS** | `feat/PB-0201-configuration-paths` | Core/Application and Infrastructure Engineering for local work; user for Git gates | 2026-07-25 | Original task commit `e64ca3a3b249163b96d08554d7ef59fa1f0c44c7` merged through PR #28 as `c89d5afab69540e640e7f4d1dfd2215c64015ada`, but PR run `30167796737` and required `main` run `30167799680` failed because one CWD-independence test assumed the local-only directory `C:\Dev\PackageBuilder\runtime-data` existed on GitHub's `D:\a\...` checkout. The narrowly scoped portable test correction is fully validated locally: the regression passes, all 53 Application tests pass, all 1,239 core tests pass, Debug/Release builds have zero warnings/errors, full Core CI passes all 9 stages, Application configuration coverage remains 100% line/branch, and no vulnerable package is reported. Production behavior is unchanged. The user approved a one-time PB-0201 corrective-publication exception on 2026-08-03 for exactly the test and synchronized PB-0201 evidence/status documents; it creates no precedent. | User-controlled corrective commit, branch push, corrective merge and `main` push, successful required `main` CI, and explicit completion confirmation remain. |
