"""Strict protocol-v1 request, progress, and result helpers shared by Python engine workers."""
...
def validate_request(value: Any) -> dict[str, Any]:
    ...
    if "target" in value and (
        not isinstance(value["target"], str) or value["target"] not in _TARGETS
    ):
...
def reject_linked_path(path: Path) -> None:
    """Reject symlinks/junctions at every existing path boundary before accessing data."""
    for entry in (path, *path.parents):
        if entry.is_symlink() or (
            entry.exists()
            and getattr(entry.lstat(), "st_file_attributes", 0) & stat.FILE_ATTRIBUTE_REPARSE_POINT
        ):
            raise WorkerInputError("A worker path contains a link.")
...
def resolve_logical_reference(workspace: Path, reference: str) -> Path:
    ...
    reject_linked_path(root / Path(*reference.split("/")))
