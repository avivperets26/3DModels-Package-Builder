...
Invoke-Check 'Contribution documentation passes dependency-free validation' {
    $validatorPath = Join-Path $script:RepositoryRoot 'scripts\Test-ContributionDocumentation.ps1'
    if (-not (Test-Path -LiteralPath $validatorPath -PathType Leaf)) {
        throw 'Missing scripts/Test-ContributionDocumentation.ps1.'
    }

    & $validatorPath -RepositoryRoot $script:RepositoryRoot
}

Invoke-Check 'Contribution documentation validator supports standalone Windows PowerShell invocation' {
    $validatorPath = [System.IO.Path]::GetFullPath(
        (Join-Path $script:RepositoryRoot 'scripts\Test-ContributionDocumentation.ps1')
    )
    if (-not (Test-Path -LiteralPath $validatorPath -PathType Leaf)) {
        throw 'Missing scripts/Test-ContributionDocumentation.ps1.'
    }
    ...
    if ($validatorExitCode -ne 0) {
        $capturedOutput = $validatorOutput -join [Environment]::NewLine
        throw "Standalone contribution-documentation validator failed with exit code ${validatorExitCode}. Captured output:`n$capturedOutput"
    }
}

Invoke-Check 'GitHub governance passes dependency-free validation' {
    $validatorPath = Join-Path $script:RepositoryRoot 'scripts\Test-GitHubGovernance.ps1'
    if (-not (Test-Path -LiteralPath $validatorPath -PathType Leaf)) {
        throw 'Missing scripts/Test-GitHubGovernance.ps1.'
    }

    & $validatorPath -RepositoryRoot $script:RepositoryRoot
}

Invoke-Check 'GitHub governance validator supports standalone Windows PowerShell invocation' {
    $validatorPath = [System.IO.Path]::GetFullPath(
        (Join-Path $script:RepositoryRoot 'scripts\Test-GitHubGovernance.ps1')
    )
    if (-not (Test-Path -LiteralPath $validatorPath -PathType Leaf)) {
        throw 'Missing scripts/Test-GitHubGovernance.ps1.'
    }
    ...
    if ($validatorExitCode -ne 0) {
        $capturedOutput = $validatorOutput -join [Environment]::NewLine
        throw "Standalone GitHub-governance validator failed with exit code ${validatorExitCode}. Captured output:`n$capturedOutput"
    }
}

Invoke-Check 'Architecture decision records pass dependency-free validation' {
    $validatorPath = Join-Path $script:RepositoryRoot 'scripts\Test-ArchitectureDecisionRecords.ps1'
    if (-not (Test-Path -LiteralPath $validatorPath -PathType Leaf)) {
        throw 'Missing scripts/Test-ArchitectureDecisionRecords.ps1.'
    }

    & $validatorPath -RepositoryRoot $script:RepositoryRoot
}

Invoke-Check 'Architecture decision record validator supports standalone Windows PowerShell invocation' {
    $validatorPath = [System.IO.Path]::GetFullPath(
        (Join-Path $script:RepositoryRoot 'scripts\Test-ArchitectureDecisionRecords.ps1')
    )
    if (-not (Test-Path -LiteralPath $validatorPath -PathType Leaf)) {
        throw 'Missing scripts/Test-ArchitectureDecisionRecords.ps1.'
    }
    ...
    if ($validatorExitCode -ne 0) {
        $capturedOutput = $validatorOutput -join [Environment]::NewLine
        throw "Standalone ADR validator failed with exit code ${validatorExitCode}. Captured output:`n$capturedOutput"
    }
}
...
Invoke-Check 'Task branch names and lifecycle markers are valid' {
    ...
    if (Test-PackageBuilderTaskBranch 'PB-1113' 'feat/PB-1110-PB-1112-unreal-overview-validation') {
        throw 'Unreal overview exception expanded beyond its approved scope.'
    }

    foreach ($id in @('PB-1107', 'PB-1108', 'PB-1109')) {
        if (-not (Test-PackageBuilderTaskBranch $id 'feat/PB-1107-PB-1109-unreal-materials-meshes')) {
            throw 'Approved Unreal surface branch rejected.'
        }
    }
    if (Test-PackageBuilderTaskBranch 'PB-1110' 'feat/PB-1107-PB-1109-unreal-materials-meshes') {
        throw 'Unreal surface exception expanded beyond its approved scope.'
    }
    ...
    if ((Test-PackageBuilderTaskBranch 'PB-0904' 'codex/PB-0901-PB-0903-documentation') -or
        (Test-PackageBuilderTaskBranch 'PB-0901' 'codex/PB-0901-PB-0904-documentation')) {
        throw 'The documentation branch exception expanded beyond its approved scope.'
    }
...
Invoke-Check 'Versioned Unity project template is minimal and deterministic' {
    $validatorPath = Join-Path $script:RepositoryRoot 'scripts\Test-UnityProjectTemplate.ps1'
    if (-not (Test-Path -LiteralPath $validatorPath -PathType Leaf)) {
        throw 'Missing scripts/Test-UnityProjectTemplate.ps1.'
    }
